...
function NextActions() { return <section className="border-t border-zinc-800 bg-zinc-900/30 py-12"><div className="mx-auto max-w-7xl px-4 lg:px-8"><h2 className="mb-2 text-2xl font-bold text-white">Next Actions Queue</h2><p className="mb-6 max-w-3xl text-sm text-zinc-500">The DXA Index should always make the next move visible. These are the items that need action before the pipeline can move forward.</p><div className="space-y-2">{nextActions.map((action, index) => <div key={action} className="flex items-center gap-3 rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-red-500/10 text-xs font-semibold text-red-500">{index + 1}</span><p className="text-sm text-zinc-300">{action}</p></div>)}</div></div></section> }
